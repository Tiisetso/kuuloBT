...
void	run_diagnostics(t_app *app)
{
	...
	printf("  %s Run " C_CYN "blueconnect connect" C_RST
		" to connect AirPods\n", SYM_INFO);
	printf("\n");
}
...
void	fix_audio_config(void)
{
	...
	printf("\n" C_BOLD C_YEL "  ── Applying Audio Fixes ──" C_RST "\n\n");
	...
	if (fp)
	{
		fprintf(fp, "# Generated by blueconnect\n");
	...
	printf("\n  %s Fixes applied. Now run " C_CYN "blueconnect connect"
		C_RST "\n\n", SYM_OK);
}
